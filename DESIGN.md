...
### 1.3.1 `image_gen` 的两个实测约束（能力卡已写入）

真实运行暴露了内置工具的两个固有特性，它们不属于缺陷，但调用方必须知道：

| 约束 | 实测 |
|---|---|
| **实际输出尺寸不等于请求尺寸** | 请求 `1024x1024`，实际得到 `1254x1254`。请求值只能作为构图意图传达 |
| **平涂区域带抗锯齿色差** | 边缘像素不是精确纯色；声称"精确纯色"是不成立的 |

批量场景还多一条：**同一套素材的视觉一致性只能做到近似**。实测三个图标成套产出时，圆角半径、底板尺度、主体占比、细节线宽在不同张之间有肉眼可见的偏差——agent 自己也如实报告了这一点（"底板边距、主体尺寸和细节线宽也只是近似一致"）。

这三条已写入能力卡，并明确要求 agent **如实报告而不为凑指标反复重试**。
...
| **L-7** | **高** | **`codex_skill_verify` 验证的不是那张卡**：它调用 `doRun` 时**漏传 `card`**，于是落入 ad-hoc 分支、用泛化文本当任务执行；而 `verify: true` 又恰好绕过了「needs-review 不得自动运行」的护栏。沙箱也因此取自调用方而非卡片——账本里出现 `sandbox: danger-full-access`，而卡片声明的是 `workspace-write`。这个检查**在报告另一件事的结果，却看起来像通过** | 真实实例里调用后，账本记录与卡片声明不符 | 预检与执行两处都转发 `card`。新增回归测试断言**卡片自身的提示词文本确实进了子进程 stdin**，且**沙箱取自卡片** |
| **L-8** | **中** | **默认探针无法通过卡片自己的预检**：`defaultProbeInputs` 只为**声明了 `default`** 的输入填值，而 `image.generate` 的 `prompt` 是必填无默认。于是「验证」在零成本预检就报 `MISSING_INPUTS`，永远到不了真实调用。生成类能力几乎都是这个形状，属常见路径而非边界 | 真实实例里第一次调用 `codex_skill_verify` | 探针值来源扩为：卡片 `default` → 输入上的 `probe:` 声明 → 按名称的通用回退；仍有缺口时追加第二轮，让验证给出**真实诊断**而不是预检拒绝 |
| **L-9** | **低** | 报错措辞误导：进程已运行一段时间后失败，仍报 `failed to start` | 调试 L-7 的过程中读到 | 按已运行时长区分「未能启动」与「运行后失败」 |
| **L-10** | **中** | **制品被重复导入**：卡片要求 Codex 自己把产物复制进工作区并用语义化文件名，Codex 照做了；控制器**又**把 Codex 侧的源文件复制一份，同一份字节存了两份，而 `artifacts` **只报告了控制器那份哈希名副本**，真正的交付物反而缺席 | 真实出图后检查磁盘，发现两个同大小 PNG | 工作区目标目录里已存在同大小文件的源不再重复导入；`artifacts` 改为「agent 自己放的文件优先」，并新增 `recovered` 字段区分控制器补收的部分 |
...
5. **测试框架本身会撒谎。** 把 `async` 回调传给同步的测试辅助函数时，Promise 不被 await，断言变成未捕获拒绝：输出显示「全部通过」，进程随后崩溃。**先让框架会失败，再相信它的通过**——这一条修好之后，同一个提交里立刻有一个此前「绿」的测试暴露出真实缺陷（L-10）。
...
| 能力 | 状态 | 证据 |
|---|---|---|
| `image.generate` | ✅ 已端到端验证 | 真实出图并回收进工作区；`artifacts` 报告语义化文件名、`recovered: []`、磁盘恰好 1 份 |
| `art.assets` | ✅ 已端到端验证（输出为候选） | 一次会话 3 张，640 857 ms；风格锚点被正确计算并复用，3 张视觉一致但**非严格一致**（见 §1.3.1），agent 如实报告了偏差 |
| `design.spec` | ⏳ 未验证 | 纯文本，成本最低 |
| `code.review` | ⏳ 未验证 | 需 git 工作区有可评审差异 |
