...
```bash
npm run selftest     # 58 项离线检查：不联网、不调用 Codex
...
|---|---|
| 离线自测 | **58 / 58** |
| 真实端到端 | **9 / 9**（含出图：真实产出 PNG 并回收进工作区） |
| 注册与路由（quick） | **5 / 5** |

离线自测覆盖了一批评审回归：种子卡 frontmatter 完整性、运行副作用导致的自我死锁、用户文件冒充所有权、制品路径逃逸、超时误判为成功、危险沙箱授权边界、重复 id、新卡状态、多行序列化。
