...
function createWorkers(deps) {
  ...
  async function skillVerify(args = {}) {
    ...
    const attempted = []
    for (const inputs of probe) {
      // `card` MUST be forwarded: without it doRun falls into the ad-hoc branch,
      // so the "verification" would run a generic task instead of the card's own
      // prompt — and `verify: true` would suppress the guard that exists to stop
      // exactly that. The check would then report on something other than the
      // capability being verified.
      const res = await doRun({ ...args, workspace: ws, card, capability: card.id, inputs, verify: true })
  ...
  async function doRun(args) {
    ...
    // The project config can point at a different Codex home; resolve it here so
    // it can be forwarded alongside the run.
    const config = codexRun.readProjectConfig(ws)
    const result = await codexRun.run(
      {
        ...
        // Forwarded explicitly so artifact patterns and reported paths are
        // expanded against the SAME Codex home the run used. Without it the
        // recovery layer silently fell back to the default home.
        codexHome: args.codexHome || config.codexHome,
...
/**
 * Build the probe inputs used when a caller does not supply `probeInputs`.
 *
 * Every REQUIRED input must get a value, or the probe cannot pass the card's own
 * pre-flight: `image.generate` requires `prompt` with no default, so probing it
 * with declared defaults alone failed with MISSING_INPUTS and never reached a
 * real call. A capability that generates something will almost always have such
 * an input, so this is the common case, not the edge case.
 *
 * Values come from, in order: the card's declared default, a probe value the
 * card itself declares (`probe:` on the input), then a generic fallback by name.
 */
const PROBE_FALLBACKS = {
  prompt: '一个简单的深色背景方形图标，纯色几何形状，无文字',
  task: '用最小代价验证该能力可用：执行一次最简单的真实调用，不要额外发挥。',
  brief: '一个用于验证能力可用性的最小项目',
  items: '一个图标',
  scope: 'uncommitted',
  focus: '仅验证能力可用，不必深入',
}
...
function defaultProbeInputs(card) {
  ...
  for (const name of missing) {
    if (PROBE_FALLBACKS[name] !== undefined) values[name] = PROBE_FALLBACKS[name]
  }
  const probes = [{ ...values }]
  // A second attempt covering any remaining required input we have no value for,
  // so a verification reports a real diagnosis instead of a pre-flight refusal.
  const stillMissing = missing.filter((name) => values[name] === undefined)
  if (stillMissing.length > 0) {
    const second = { ...values }
    for (const name of stillMissing) second[name] = PROBE_FALLBACKS.task
    probes.push(second)
  }
  return probes
}
