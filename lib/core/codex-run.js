...
async function run(request, spawn) {
  ...
  try {
    ...
    const enriched = new Error(`codex exec failed to start: ${error.message}`)
    enriched.code = error.code || 'SPAWN_FAILED'
  ...
  // reported. Never synthesise a path from the thread id (DESIGN.md §12.1 D1).
