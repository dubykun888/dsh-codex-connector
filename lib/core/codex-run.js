...
async function run(request, spawn) {
  ...
  if (request.artifacts && request.artifacts.patterns) {
    ...
    // When the agent followed the card and copied its own output into the
    // workspace, importing the Codex-side SOURCE too would store the same bytes
    // twice under two names. Comparing by BASENAME is not enough: the card asks
    // for a semantic name (`hero-boat.png`) while the Codex-side file carries a
    // hash name (`exec-abc.png`), so the two never match. The import is detected
    // by finding a file of identical size in the destination directory, which is
    // what "the agent already put this here" actually looks like.
    ...
    const toImport = [...fresh].filter((file) => !alreadyImported(file))
