...
/**
 * Tool return values must be LOSSLESS JSON. The harness rejects anything else:
 *
 *   tool "codex_do" returned invalid output: value is not lossless JSON
 *
 * Worker results contain `undefined` in several honest places — a missing
 * `threadId`, an artifact collection with no destination after a refusal, a
 * capability with no health record yet. Any one of them makes the whole result
 * unrepresentable, and the failure happens at the tool boundary, AFTER the work
 * is done. Measured: it only appears when the result crosses the tool boundary,
 * so calling a worker directly never showed it.
 *
 * The rules below mirror the harness validator exactly rather than approximating
 * it — it rejects more than "undefined" does:
 *   - numbers must be finite and are not -0
 *   - arrays must be plain and carry no extra own keys
 *   - objects must be plain, and a property whose value is not representable is
 *     DROPPED (an explicit null would claim the value is known to be empty)
 *   - undefined / function / symbol / bigint cannot cross at all
 *   - cycles cannot be represented; they are broken with a marker instead of
...
function toLosslessJson(value, seen = new Set()) {
  ...
  if (type === 'number') return Number.isFinite(value) && !Object.is(value, -0) ? value : null
  ...
  try {
    if (Array.isArray(value)) {
      ...
      for (let i = 0; i < value.length; i += 1) {
        const next = toLosslessJson(value[i], seen)
        // A hole or an unrepresentable item becomes null, so the array keeps
        // its length and no extra own keys appear.
    ...
    for (const [key, item] of Object.entries(value)) {
      const next = toLosslessJson(item, seen)
...
const TOOL_SPECS = [
  ...
  },
  {
    ...
    parameters: {
      // NOT required: a capability card supplies its own task text, so
      // `{ capability, inputs }` is a complete call. Declaring it required made
      // that call fail argument validation, and the harness reported the failure
      // as an output problem ("value is not lossless JSON"), which pointed at
      // entirely the wrong layer. The routing layer enforces the real rule.
      task: {
        type: 'string',
        description:
          'What Codex should do, in natural language. Required unless `capability` is given (a card carries its own task text).',
      },
  ...
  function apply(ctx) {
    ...
    for (const spec of TOOL_SPECS) {
      ...
      const definition = {
        ...
        parameters,
        // Keep the per-call ceiling above the longest run a capability card may
        // legitimately start (the batch card declares up to 30 minutes), so the
        // harness never kills a run that is still making progress. A missing
        // ceiling here would hang or truncate long calls from the outside.
        timeoutMs: 3_600_000,
        output: { schema: {}, render: renderJson },
        async execute(args, exec) {
          try {
            // Everything crossing the tool boundary is normalised to lossless
            // JSON; see toLosslessJson for why `undefined` cannot be returned.
            return toLosslessJson(await invoke(spec.worker, args, exec))
          } catch (error) {
            return {
              ok: false,
